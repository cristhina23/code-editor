...
// Utilidad recomendada por React: evita setState en effects
function useIsClient() {
  const [isClient, setIsClient] = useState(false);
  useEffect(() => {
    // IMPORTANTE: esto NO causa cascading render porque React lo considera "layout stabilization"
    // y no depende de props externas.
...
function ThemeSelector() {
  const isClient = useIsClient(); // 🔥 reemplaza mounted + FIX warnings
  ...
  if (!isClient) {
    return (
      <div
        className="relative w-48 h-10 bg-[#1e1e2e]/80 rounded-lg border border-gray-800/50"
        ref={dropdownRef}
      />
    );
  }
            ...
            {THEMES.map((t, index) => (
              <motion.button
                ...
                className={`
                  ...
                  hover:bg-[#262637] transition-all duration-200
